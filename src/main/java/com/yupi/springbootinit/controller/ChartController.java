...
import com.yupi.springbootinit.bimq.BiMessageProcuder;
...
public class ChartController {
    ...
    @Resource
    private BiMessageProcuder biMessageProcuder;
...
//                "【【【【\n" +
//                "{前端Echarts V5 的option 配置对象js代码，合理地将数据进行可视化，不要生成多余的内容，比如注释}\n" +
//                "【【【【\n" +
//                "{明确的数据分析结论，越详细越好，不要生成多余的注释}";

        Long biModelId = 1659171950288818178L;
        /**
         * userInput: 用户的输入（prompt）
         */
        // 用户输入
...
//        }
    ...
    public BaseResponse<BiResponse> genChartByAiAsync(@RequestPart("file") MultipartFile multipartFile,
        ...
        ThrowUtils.throwIf(!validFileSuffixList.contains(suffix), ErrorCode.SYSTEM_ERROR, "非法的文件后缀");
...
//                "{明确的数据分析结论，越详细越好，不要生成多余的注释}";
        ...
        Long biModelId = CommonConstant.BIMODELID;
        // 用户输入
        StringBuilder userInput = new StringBuilder();
        //系统预设
        userInput.append("分析需求：").append("\n");
        ...
        ThrowUtils.throwIf(!saveResult,ErrorCode.SYSTEM_ERROR,"图标保存失败");
        long newChartId = chart.getId();
        biMessageProcuder.sendMessage(String.valueOf(newChartId));
        BiResponse biResponse = new BiResponse();
        biResponse.setChartId(chart.getId());
        return ResultUtils.success(biResponse);
...
//        }
     ...
     * @return
     */
    private QueryWrapper<Chart> getQueryWrapper(com.yupi.springbootinit.model.dto.chart.ChartQueryRequest chartQueryRequest) {
